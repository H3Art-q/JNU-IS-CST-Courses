...
Type the following in the Linux console/shell to compile and make an executable
using the gcc complier-
...
int main() {
    ...
    fprintf(my_file_pointer,
            "The sizes of different data type for this machine and compiler "
            "are -\n");
    ...
    printf("int data type is %d bytes or %d bits long\n", sizeof(int_var),
           sizeof(int_var) * 8);  // Tag 2
    printf("double data type is %d bytes or %d bits long\n", sizeof(double),
           sizeof(double) * 8);

    fclose(my_file_pointer);  // To close the output file, mandatory to actually
                              // get an output !
